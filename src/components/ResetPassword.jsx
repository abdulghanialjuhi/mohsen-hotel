...
export default function ResetPassword() {
    ...
    const handleLogIn = async (e) => {
        ...
        try {
            ...
            setTimeout(() => setErorrMessage(''), 3000)
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className='flex-grow flex justify-center items-center'>
            <div className='w-full max-w-[650px] min-h-[430px] shadow my-8 bg-gray-0 rounded flex flex-col items-center p-3'>
                ...
                <div className='min-h-[1rem] mt-2 flex justify-center max-w-[450px]'>
                    ...
                    {successMessage && <span className='h-full bg-green-500 py-1 text-center px-2 rounded-md text-gray-100'> {successMessage} </span>}
                </div>
                    ...
                    <form onSubmit={handleLogIn} className='w-full h-full'>
                        <div className='w-full max-w-[350px] mx-auto mt-4 flex flex-col gap-5'>
                            ...
                            <div className='flex mt-4 justify-end'>    
                                <input disabled={loading} type="submit" name="submit" className='p-2 px-3 rounded bg-primaryBlue text-gray-0 cursor-pointer w-full' value={loading ? 'sending...' : 'Send an email'} />
                            </div>
                        </div>
                    </form>
